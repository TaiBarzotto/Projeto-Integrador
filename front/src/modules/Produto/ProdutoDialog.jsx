...
export function ProdutoDialog({ open, onOpenChange, produto, onSave }) {
  ...
  const buscaCategoriasProdutos = async () => {
    try {
      const response = await axios.get('http://localhost:3002/produto/categorias')
  ...
  const handleGerarVariacoes = () => {
    const tamanhos = tipoTamanho === 'letras' ? TAMANHOS_PADRAO : TAMANHOS_NUMERICOS
  ...
  const handleUpdateFornecedoresVariante = (index, fornecedorIds) => {
    const fornecedoresSelecionados = fornecedoresCadastrados.filter(f => 
      fornecedorIds.includes(f.id)
    )
    handleUpdateVariante(index, 'fornecedores', fornecedoresSelecionados)
  }
                ...
                renderValue={(selected) => (
                  <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                    {selected.map((id) => {
                      ...
                      return <Chip key={id} label={cat?.nome || id} size="small" />
                    })}
                  </Box>
                )}
              >
        ...
        <TabPanel value={tabAtual} index={1}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            <Typography variant='body2' color='text.secondary'>
              Use esta ferramenta para gerar rapidamente múltiplas variantes combinando cores e tamanhos
            </Typography>
            ...
            <Box sx={{ pt: 2, borderTop: 1, borderColor: 'divider' }}>
              ...
              <Typography variant='caption' color='text.secondary' sx={{ mt: 1, display: 'block' }}>
                Tamanhos: {tipoTamanho === 'letras' ? TAMANHOS_PADRAO.join(', ') : TAMANHOS_NUMERICOS.join(', ')}
              </Typography>
            </Box>
            ...
            <Box>
              ...
              <Typography variant='caption' color='text.secondary' sx={{ mt: 1, display: 'block' }}>
                Isso criará {coresDisponiveis.length} cores × {tipoTamanho === 'letras' ? TAMANHOS_PADRAO.length : TAMANHOS_NUMERICOS.length} tamanhos = {coresDisponiveis.length * (tipoTamanho === 'letras' ? TAMANHOS_PADRAO.length : TAMANHOS_NUMERICOS.length)} variantes
              </Typography>
            </Box>
          </Box>
        </TabPanel>
                    ...
                    <TableRow key={index}>
                      <TableCell>
                        <TextField
                          size='small'
                          value={variante.cor || ''}
                          onChange={e => handleUpdateVariante(index, 'cor', e.target.value)}
                          placeholder='Cor'
                          sx={{ minWidth: 100 }}
                        />
                      </TableCell>
                      <TableCell>
                        <TextField
                          ...
                          onChange={e => handleUpdateVariante(index, 'tamanho', e.target.value)}
                          placeholder='Tamanho'
                          sx={{ minWidth: 80 }}
                        />
                      </TableCell>
                      <TableCell>
                        <TextField
                          ...
                          onChange={e => handleUpdateVariante(index, 'codigo_de_barras', e.target.value)}
                          placeholder='Código'
                          sx={{ minWidth: 130 }}
                        />
                      </TableCell>
                      <TableCell>
                        <TextField
                          ...
                          onChange={e => handleUpdateVariante(index, 'custo', e.target.value)}
                          placeholder='0.00'
                          sx={{ width: 90 }}
                        />
                      </TableCell>
                      <TableCell>
                        <TextField
                          ...
                          onChange={e => handleUpdateVariante(index, 'quantidade_estoque', parseInt(e.target.value) || 0)}
                          sx={{ width: 80 }}
                        />
                      </TableCell>
                      <TableCell>
                        <TextField
                          ...
                          onChange={e => handleUpdateVariante(index, 'limite_minimo', parseInt(e.target.value) || 0)}
                          sx={{ width: 80 }}
                        />
                      </TableCell>
                      <TableCell>
                        <FormControl size='small' sx={{ minWidth: 150 }}>
                          <Select
                            multiple
                            value={variante.fornecedores?.map(f => f.id) || []}
                            onChange={e => handleUpdateFornecedoresVariante(index, e.target.value)}
